...
func (m *MemUserStore) CheckWhetherFollowingDB(srcId uint, targetId uint, result chan bool,
	...
	if !ok || !ok2 {
		errChan <- &storage.MyStorageError{Message: "Non-exist User ID"}
		return
	}
...
func (m *MemUserStore) StartFollowingDB(srcId uint, targetId uint, result chan bool,
	...
	if !ok || !ok2 {
		errorChan <- &storage.MyStorageError{Message: "Non-exist User ID"}
		return
	}
	for e := srcUser.Following.Front(); e != nil; e = e.Next() {
		fuid := e.Value.(uint)
	...
	newSrcUser := storage.UserEntity{}
	newTarUser := storage.UserEntity{}
